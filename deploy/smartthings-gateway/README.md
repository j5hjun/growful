...
5. `127.0.0.1:8100`의 `/healthz`, 인증 없는 `/connection`, `/oauth/start`, `/v1/*` 확인
   - 공개 `GET /oauth/start` 선택 화면과 유효한 `POST /oauth/start` 리다이렉트를 함께 확인
...
안의 부분집합만 요청합니다. 앱 등록에는 `r:devices:$`,
`x:devices:$`, `w:devices:$`, `r:devices:*`, `x:devices:*`, `w:devices:*`,
`r:locations:*`를 허용합니다.

기존 토큰에 저장된 다른 등록 scope는 OAuth 표준 scope 문자열이면 계속 읽고 표시하며,
refresh에서는 기존 승인 범위의 부분집합만 허용합니다. 새 권한 선택 화면에서는 위의 서버
허용 목록 밖 scope를 새로 요청할 수 없습니다.
...
2. `https://smartthings.growful.click/oauth/start`에서 기능·디바이스 범위를 고른 뒤
   SmartThings 화면에서 실제 디바이스를 선택해 승인
3. callback 완료 화면에 한 번 표시되는 Growful 토큰을 안전하게 보관
4. 로컬 터미널에서 Growful 토큰으로 연결 상태와 실제 프록시를 확인
...
5. 두 SmartThings 연결에서 각각 발급한 토큰이 서로 다른 연결을 사용하는지 확인
6. 갱신 예정 시간이 지난 뒤 `/connection`의 `lastRefreshedAt`과 `expiresAt` 확인
