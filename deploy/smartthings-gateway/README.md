...
5. `127.0.0.1:8100`의 `/healthz`, `/connection`, `/oauth/start`, 인증 없는 `/v1/*` 확인
   - 인증된 `GET /oauth/start` 선택 화면과 유효한 `POST /oauth/start` 리다이렉트를 함께 확인
...
배포 스크립트가 이전 버전 기동에만 최소 호환값을 임시 주입하고, 운영 `.env`는 변경하지
않습니다. 앱 등록 시 허용한 scope가 최종 상한이고 Gateway 선택 화면은 서버 허용 목록
...
3. `https://smartthings.growful.click/oauth/start`에서 HTTP Basic 비밀번호로
   `OAUTH_ADMIN_TOKEN`을 입력하고, Gateway에서 기능·디바이스 범위를 고른 뒤
   SmartThings 화면에서 실제 디바이스를 선택해 승인
4. callback 완료 후 `/connection`의 `connected`, `grantedScopes`, `expiresAt` 확인
