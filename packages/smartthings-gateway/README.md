...
- `GET /healthz`: 프로세스 상태
- `GET /oauth/start`: HTTP Basic 인증 후 권한·디바이스 범위 선택 화면
- `POST /oauth/start`: 선택값 검증 후 SmartThings 권한 승인 시작
- `GET /oauth/callback`: authorization code 교환
- `GET /connection`: 연결 여부, 승인 scope, 만료 시각, 마지막 갱신 시각
...
`OAUTH_ADMIN_TOKEN`을 입력합니다. Gateway 화면에서 기능과 디바이스 범위를 고른 뒤
SmartThings 동의 화면에서 실제 디바이스를 선택합니다. 허용 scope는 서버 코드에 고정되어
