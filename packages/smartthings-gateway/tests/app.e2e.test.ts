...
describe("SmartThings Gateway HTTP API", () => {
  it("issues a Growful token once and requires it for connection status", async () => {
    ...
    expect(callbackResponse.headers["content-security-policy"]).not.toContain("script-src 'none'")
    expect(callbackResponse.headers["cross-origin-opener-policy"]).toBe("same-origin")
    expect(callbackResponse.headers["cross-origin-resource-policy"]).toBe("same-origin")
    expect(callbackResponse.headers["permissions-policy"]).toBe(
    ...
    expect(callbackResponse.headers["x-frame-options"]).toBe("DENY")
    expect(callbackResponse.body).toContain('<meta name="robots" content="noindex,nofollow">')
    ...
    expect(callbackResponse.body).not.toContain("authorization-code")
    expect(callbackResponse.body).not.toContain("test-state-with-sufficient-entropy")
