...
모든 사용자용 HTML은 최대 64rem인 하나의 `page-shell`을 공유한다. 페이지를 전환해도
header와 shell의 좌우 경계는 바뀌지 않으며, shell 안의 `main`만 용도별로 중앙 정렬한다.
랜딩·넓은 본문은 64rem, 연결 관리·상태·지원·개인정보·약관·브라우저 404는 42rem,
OAuth 권한 선택·완료·오류와 비공개 베타 접근 안내는 34rem이다. `data-body-width`는 이
선택을 테스트 가능한 문서 계약으로 노출하되 비밀값이나 사용자 식별자를 포함하지 않는다.

화면 좌우 gutter는 1280px에서 32px, 768px에서 24px, 375px에서 16px, 320px에서
12px이다. header 최소 높이는 데스크톱 64px, 40rem 이하 모바일 56px이며 메뉴가 줄바꿈되면
콘텐츠에 따라 자동으로 늘어난다. 페이지 제목 영역은 데스크톱 상하 48px, 모바일 상하 24px,
섹션 간격은 데스크톱 32px, 모바일 24px, 카드 내부 여백은 데스크톱 24px, 모바일 16px을
사용한다. 고정 높이로 콘텐츠를 자르지 않는다. navigation, footer, 행동 그룹과 3단계 흐름은
40rem 이하에서 한 열로 전환하며 메뉴 항목은 DOM 순서를 유지한 채 자연스럽게 줄바꿈한다.

본문 최대 너비 토큰은 `--panel-max` (`34rem`), `--panel-manage` (`42rem`),
`--panel-wide` (`64rem`), shell 반경은 `--radius-panel` (`.75rem`),
...
문서의 scroll padding은 상단 안전 여백과 행동 영역 높이를 포함한다. 모든 포털, OAuth,
비공개 베타 HTML 화면은 콘텐츠 길이와 viewport 높이에 관계없이 같은 상단 safe-area와
64rem shell 기준선을 사용한다.
...
- Structure: skip link → 단일 semantic site `header` → 단일 `main#main-content` → 단일 `footer`
- Responsibility: 완전한 문서를 한 번에 렌더링하고 공통 64rem shell, 본문 폭, semantic token,
  typography, phrase wrapping, responsive layout, dark/forced-colors를 제공
- Header variants: 일반 포털은 브랜드·서비스 안내·연결 관리·상태·지원, OAuth·비공개 베타는
  브랜드·`SmartThings 연결` 현재 문맥·서비스 안내·지원을 제공. 완료 화면의 이탈 링크는
  `토큰 저장 후` 문구로 one-time token 저장 전 위험을 명시
- Extension: 각 페이지는 본문과 component-specific styles, 본문 폭, header variant/current
  state만 제공하며 문서 문자열 치환으로 landmark를 삽입하지 않음
- Boundary: `/healthz`, `/readyz`, `/v1`, `/connection`, `/token`, 정적 JS, `robots.txt`와
  업스트림 `text/html` passthrough에는 이 shell을 적용하지 않음. CSP는 scope, completion,
  callback error, portal/manage별 route 정책을 그대로 유지하고 shell에서 합치지 않음
...
- Validation: 서버 검증 실패 시 허용 목록에 있는 디바이스 범위·리소스 권한·정책 동의 draft를
  그대로 유지하고, 처리할 수 없는 값과 알 수 없는 필드는 재출력하지 않는다. 유효하지 않거나
  모호한 디바이스 범위는 최소 권한 기본값인 `selected`로 복구한다.
- Motion: 없음
...
- States: 기본, details 열림·닫힘, 체크됨, 키보드 focus, 잘못된 선택값, 권한 누락,
  정책 동의 누락
- Accessibility: 각 리소스 이름을 `legend`로 제공하고 scope 문자열은 label 안의 설명으로 연결
- Validation semantics: 오류 요약은 `role="alert"`, `tabindex="-1"`, `autofocus`를 사용해
  서버 응답 직후 안내로 포커스를 옮긴다. 이는 fresh load의 첫 Tab skip-link 계약에 대한
  의도적 예외다. Shift+Tab으로 header를 역순 이동해 skip link에 도달할 수 있고, 오류 요약에서
  Tab하면 복구 입력 순서로 이어진다. 권한 누락은 모든 리소스 fieldset을 감싼 단일
  `role="group"`에 `aria-invalid`와 요약을 연결하고, 정책 동의 누락은 checkbox에 같은 요약을
  연결한다. 잘못된 선택값은 원본 요청이나 식별자를 노출하지 않는 일반 안내만 제공한다.
- Responsive: 375px부터 한 열을 유지하며 문서 세로 스크롤만 사용한다. 200% 확대의 유효 폭에서는
  패널·fieldset·요약의 안전 여백을 한 단계 줄이고 긴 scope를 줄바꿈해 가로 스크롤을 만들지 않는다.
- Motion: 없음
...
- Accessibility: 네이티브 required checkbox와 명시적 label을 사용하고 정책 링크의 접근 가능한
  이름으로 새 탭 동작을 알림. 제출된 값이 `accepted`이면 다른 검증 오류가 있어도 체크 상태를
  유지하며, 누락 시 오류 요약과 `aria-describedby`로 연결한다.
...
- Structure: `header.site-header > nav[aria-label] > a.brand + ul > li`
- Primary links: 서비스 안내, 연결 관리, 상태, 지원을 상단에 유지
- Secondary links: 개인정보 처리방침, 이용약관은 공통 footer의 보조 navigation으로 제공하고,
  운영자명과 `mailto:` 지원 이메일을 같은 footer에 표시
- States: 기본, hover, focus-visible, 현재 페이지. 일반 문서는 header/footer 전체에서 정확히
  하나의 `aria-current="page"`를 사용하고 404는 일치하는 메뉴가 없으므로 사용하지 않음.
  작업 header는 링크가 아닌 `SmartThings 연결` 문맥에 현재 상태를 표시
- Accessibility: 문서 첫 포커스 항목은 `main#main-content`를 가리키는 skip link이며, 목적을 설명하는
  링크 문구와 최소 44×44px 터치 영역을 사용. 정책 페이지의 현재 문서는 footer 링크에
  `aria-current="page"`로 표시. 오류 autofocus 문서는 위의 명시적 예외를 따름
- Responsive: 항목 단위 flex wrap으로 DOM 순서를 보존하고 좁은 화면에서 브랜드와 링크가
  자연스럽게 줄바꿈. header는 최소 높이만 가지며 고정 높이나 강제 잘림을 사용하지 않음
...
- Structure: 포털 navigation 뒤 `header > eyebrow + h1 + summary + mailto CTA`, 문의 유형 정의 목록,
  안전한 문의 정보, 셀프서비스 절차, 미확정 운영 경계, 운영자와 `mailto:` 지원 연락처
- Content: 원시 토큰·OAuth code·state·비밀번호·원본 계정·설치 식별자·민감한 전체 응답 본문을
  요청하지 않으며, 확인된 셀프서비스 동작과 아직 확정되지 않은 본인 확인·응답시간·긴급 단계만
  사실대로 설명. 가명 `supportReference`는 안전하게 전달할 수 있는 문의 참조로 구분
- Topics: 상호작용이 없는 문의 유형은 `dl > div > dt + dd`의 텍스트 정보 구조로 제공하고,
  카드 배경·테두리·포인터·focus 상태를 적용하지 않음
- Accessibility: 단일 `main`, 순차적인 h1/h2, 문의 유형 정의 목록, 현재 문서의 `aria-current`,
  헤더 안의 명시적인 `이메일 문의하기` 링크를 제공하며 정적 문의 유형을 focus 순서에서 제외
- Responsive: CTA는 48rem 이하에서 한 열 전체 너비를 사용하고, 문의 유형과 운영자 정보는
  30rem 이하에서 한 열로 전환하며 375px과 200% 확대에서 가로 스크롤이 없어야 함
- Motion: CTA active 입력 확인 외 장식 motion 없음
...
- States: 미인증, 로딩, 연결됨·API 사용 가능, 연결됨·API 접근 차단, 교체 완료, 연결 해제됨,
  API 오류
- Action mapping: 미인증·로딩·오류에서는 토큰 제출만 표시하고, 연결됨에서는 `상태 다시 확인`을
  표시한다. 연결 해제됨에서는 입력 토큰의 `연결 상태 확인`을 유지하면서 `SmartThings 다시 연결`을
  보조 행동으로 함께 제공한다. 행동 슬롯은 두 CTA 행을 모든 상태에서 예약해 전환 시 패널 경계와
  주요 CTA의 세로 기준점을 유지한다. 연결 해제 완료 뒤 토큰 입력으로 포커스를 보내고 완료 안내를
  live status로 알린다.
...
- 모든 사용자 HTML 문서는 skip link에서 `main#main-content`로 이동하고 이후 제목·행동·footer
  순서로 예측 가능한 키보드 흐름을 제공해야 한다. 서버 오류 autofocus 예외는 위 계약대로
  역방향 skip 접근과 순방향 복구를 함께 보장한다.
- 320px, 375px, 768px, 1280px에서 가로 스크롤, 화면 밖 geometry, 강제 clip, 긴 오류 문구
  잘림이 없어야 한다. 메뉴·버튼·입력·action형 링크는 실측 최소 44×44px이다.
- DPR/축소 viewport 자동화는 `200% reflow surrogate`로만 부른다. 실제 브라우저 200% zoom은
  배포 후 수동 브라우저 gate로 별도 확인해야 하며 자동화 결과로 대체하지 않는다.
- 밝은/어두운 색상과 forced colors에서 header separator, 현재 메뉴 경계, focus ring,
  상태 정보와 404 복구 행동이 손실되지 않아야 한다.
- Accepted debt: 없음
